...
export function Header() {
  const { theme, toggleTheme, showHistory, setShowHistory, showPresets, setShowPresets } = useUIStore();
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (!data.query?.type || data.query.type !== 'group') {
        ...
        toast.error('Invalid query file');
      }
    };
    input.click();
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '20px', lineHeight: 1, color: 'var(--text-primary)' }}>
            QueryForge
          </div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--amber-dim)', letterSpacing: '0.1em' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <div style={{
          ...
          {errors.length > 0 && (
            <span style={{ color: 'var(--rose-accent)' }}>· {errors.length} error{errors.length > 1 ? 's' : ''}</span>
          )}
        </div>
      </div>
